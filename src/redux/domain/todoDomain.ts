...
type ActionType = {
  type: typeof DOMAIN_TODO_FETCH, payload: {ids: todoID[]}
}
...
export const fetch = (pagination: number) => (dispatch: any) => () => {
  ...
  dispatch({type: DOMAIN_TODO_FETCH, payload: {ids}})
  // TODO: entity
}
 ...
 * reducer
 */
export const reducer: React.Reducer<StateType, ActionType> = (state = initialState, action) => {
  switch(action.type) {
    case DOMAIN_TODO_FETCH:{
      return {...state, ids: [...state.ids, ...action.payload.ids]}
    }
    default: {
      return state
    }
